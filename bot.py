...
session_g = database_connect(config['db_name'])
db_init_admin(session_g)
tables_database_init(session_g)
db_init_users(session_g)
close_connection(session_g)
...
def handle_take(message: tt.types.Message):
    ...
    try:
        ...
        try:
            ...
            insert_value(session, {'tg_id': message.chat.id, 'time': time,
                                           'username': f"{message.from_user.first_name if message.from_user.first_name is not None else ''} {message.from_user.last_name if message.from_user.last_name is not None else ''}",
                                           'change': -1}, table_name)
            bot.send_message(message.chat.id,
...
def handle_create(message: tt.types.Message):
    ...
    if not is_logged(message.chat.id):
        return
    session = database_connect(config['db_name'])
    if not is_admin(session, str(message.chat.id)):
...
def callback_create2_handler(message: tt.types.Message, table_name: str):
    ...
    try:
        ...
        insert_table(session, {'name': table_name, 'date': dt.now(pytz.timezone('Europe/Minsk')),
                                       'time': dt.strptime(
...
def callback_query_admin_time(call: tt.types.CallbackQuery):
    ...
    try:
        ...
        bot.register_next_step_handler(call.message, callback_settime2_handler, get_table_name(session, no + 1))
        close_connection(session)
    except Exception as e:
        bot.delete_message(call.message.chat.id, call.message.message_id)
        bot.send_message(call.message.chat.id, 'Что-то не так с вводом, попробуй еще раз')
...
def callback_admin_edit3_handler(message: tt.types.Message, table_name: str, no: int):
    ...
    try:
        session = database_connect(config['db_name'])
        update_name(session, get_status_by_no(session, no, table_name)[0][1],
    ...
    try:
        ...
        if lst:
            if len(lst) >= no1 and len(lst) >= no2:
                change_queue(session, get_status_by_no(session, no1, table_name)[0],
